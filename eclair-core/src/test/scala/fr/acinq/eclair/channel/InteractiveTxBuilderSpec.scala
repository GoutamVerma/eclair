...
class InteractiveTxBuilderSpec extends TestKitBaseClass with AnyFunSuiteLike with BitcoindService with BeforeAndAfterAll {
  ...
  test("reference test vector") {
    ...
    val initiatorTx = SharedTransaction(Seq(initiatorInput), Seq(nonInitiatorInput).map(i => RemoteTxAddInput(i)), Seq(initiatorOutput, sharedOutput), Seq(nonInitiatorOutput).map(o => RemoteTxAddOutput(o)), lockTime = 120)
    assert(initiatorTx.localFees(initiatorParams) == 155.sat)
    val nonInitiatorParams = initiatorParams.copy(isInitiator = false)
    val nonInitiatorTx = SharedTransaction(Seq(nonInitiatorInput), Seq(initiatorInput).map(i => RemoteTxAddInput(i)), Seq(nonInitiatorOutput), Seq(initiatorOutput, sharedOutput).map(o => RemoteTxAddOutput(o)), lockTime = 120)
