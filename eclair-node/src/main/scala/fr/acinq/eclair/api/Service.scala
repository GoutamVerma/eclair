...
trait Service extends EclairDirectives with WebSocket with Node with Channel with Fees with PathFinding with Invoice with Payment with Message with OnChain with HardwareWalletInterface with Logging {
  ...
  def finalRoutes(extraRouteProviders: Seq[RouteProvider] = Nil): Route = securedHandler {
    val baseRoutes = nodeRoutes ~ channelRoutes ~ feeRoutes ~ pathFindingRoutes ~ invoiceRoutes ~ paymentRoutes ~ messageRoutes ~ onChainRoutes ~ webSocket ~ hwiRoute
